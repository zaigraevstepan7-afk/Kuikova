...
bool C_UserInterface::beginWindow(const char *name, bool *p_open, ImGuiWindowFlags flags)
{
    ...
    if (!ImGui::Begin(name ? name : "xxxpastuxxx_shell", nullptr,
...
void C_UserInterface::visuals()
{
    ...
    stux_two_col(L, R, h);
    if (this->beginChild("##stux_combat_a", ImVec2(L, h), ImGuiChildFlags_Borders))
    {
        stux_section("AIM");
...
void C_UserInterface::rage()
{
    float L = 0.f, R = 0.f, h = 0.f;
    stux_two_col(L, R, h);
    if (this->beginChild("##stux_sight_a", ImVec2(L, h), ImGuiChildFlags_Borders))
    {
        stux_section("ESP");
...
void C_UserInterface::misc()
{
    float L = 0.f, R = 0.f, h = 0.f;
    stux_two_col(L, R, h);
    if (this->beginChild("##stux_util_a", ImVec2(L, h), ImGuiChildFlags_Borders))
    {
        stux_section("CAMERA");
...
void C_UserInterface::exploits()
{
    ...
    if (this->beginChild(oxorany("##stux_skins"), ImVec2(w, h), ImGuiChildFlags_Borders))
    {
        stux_section("SKIN LAB");
...
void C_UserInterface::config()
{
    float L = 0.f, R = 0.f, h = 0.f;
    stux_two_col(L, R, h);
    if (this->beginChild(oxorany("##stux_sys_a"), ImVec2(L, h), ImGuiChildFlags_Borders))
    {
        stux_section("LOOK");

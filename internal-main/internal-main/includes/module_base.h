...
inline uintptr_t resolve_il2cpp_base()
{
    ...
    if (b)
    {
        __android_log_print(ANDROID_LOG_INFO, "melodium", "game base libil2cpp.so=%p", (void *)b);
        return b;
    }
    b = find_module_base_rx("libunity.so");
    if (b)
        __android_log_print(ANDROID_LOG_INFO, "melodium", "game base libunity.so=%p (Halalium path)", (void *)b);
    else
        __android_log_print(ANDROID_LOG_ERROR, "melodium", "game base NOT FOUND");
    return b;
}
